...
const requestOrder = async (
  ...
  const product = await productQuery
    .doesNotExist('deletedAt') // chỉ cho phép order product còn active
  ...
  // Guard thêm: đảm bảo còn hàng
  const remainStock = product.get('remainNumberProduct') ?? 0;
  if (remainStock <= 0) {
    throw new Parse.Error(
      Parse.Error.VALIDATION_ERROR,
      `Product ${productId} out of stock`
    );
  }
  ...
  const orderCounts = orderRequests.map(
    orderRequest => (orderRequest.get('count') as number) ?? 0
  );
  ...
  const status =
    sum(orderCounts) + count <= product.get('remainNumberProduct')
      ? OrderRequestStatus.VALID
      : OrderRequestStatus.IN_QUEUE;
  const unix = Date.now();
...
export const requestOrderGuest = async (
  ...
  const orderRequest = await queue.add<OrderRequest>(() =>
    requestOrder(productId, count)
  );
